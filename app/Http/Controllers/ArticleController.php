...
class ArticleController extends Controller
{
    ...
    public function index()
    {
       try {
        ...
        return response()->json($e->getMessage(),$e->getCode());//irajaalik num mtaa erreur 
       }
    }
    ...
    { 
        ...
        try { 
           $perPage = request()->input('pageSize', 10);  //9adeh pas men page el page Par defaut(10)
              // Récupère la valeur dynamique pour la pagination 
            $articles = Article::with('scategorie')->paginate($perPage); //paginate takhou pas 
...
public function showArticlesBySCAT($idscat) {
     try { $articles= Article::where('scategorieID', $idscat)->with('scategorie')->get(); 
        return response()->json($articles); } 
        catch (\Exception $e) 
        { return response()->json("Selection impossible {$e->getMessage()}"); } 
    }  //cette fonction taatik les articles de sous categories
